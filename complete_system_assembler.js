...
<head>
    ...
    <style>
        * { box-sizing: border-box; margin: 0; padding: 0; font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif; }
        ...
        .menu-toggle-btn { background: #0f172a; border: 1px solid #334155; color: #10b981; font-size: 14px; padding: 6px 14px; border-radius: 6px; cursor: pointer; font-weight: bold; margin-top: 10px; display: inline-block; text-transform: uppercase; font-family: monospace; }
        ...
        .nav-item { display: block; padding: 10px 12px; color: #cbd5e1; text-decoration: none; font-size: 12px; border-radius: 6px; font-weight: bold; text-transform: uppercase; font-family: monospace; transition: background 0.2s ease; border-left: 3px solid transparent; }
...
<body>
    <header>
        ...
        <button class="menu-toggle-btn" onclick="document.getElementById('sideNavMenuDrawer').classList.toggle('open')">≡ EXPAND NAVIGATION DRAWER</button>
    </header>
    ...
    <div class="sidebar-menu" id="sideNavMenuDrawer">
        ...
        <a href="/compliance" class="nav-item" style="color: #3b82f6; font-weight: bold;">⚖️ Legal Compliance Vault</a>
        <a href="/tax-vault" class="nav-item" style="color: #f59e0b; font-weight: bold;">🇰🇪 KRA Tax Compliance Ledger</a>
        <a href="/assets" class="nav-item" style="color: #a855f7; font-weight: bold;">📦 Asset Management Registry</a>
    </div>
    ...
    <div class="diag-grid" style="margin-bottom: 16px;">
        <div class="diag-card">
            <div class="diag-header">
                <span class="diag-title">📡 WIREGUARD TUNNEL INFRASTRUCTURE TRAFFIC</span>
                <span style="color: #10b981; font-weight: bold; font-size: 9px;">🟢 AUTO_REFRESH</span>
            </div>
            <div style="display: flex; justify-content: space-between; align-items: center;">
                <div><div style="color: #475569; font-size: 9px; text-transform: uppercase;">Incoming Stream</div><div class="diag-metric" id="liveRxMetric">14.2 Mbps</div></div>
                <div style="text-align: right;"><div style="color: #475569; font-size: 9px; text-transform: uppercase;">Outgoing Stream</div><div class="diag-metric" id="liveTxMetric" style="color: #3b82f6;">4.8 Mbps</div></div>
            </div>
        ...
        <div class="diag-card">
            <div class="diag-header">
                <span class="diag-title">🚨 LIVE PLATFORM SECURITY FIREWALL AUDIT LOGS</span>
...
fs.writeFileSync(dashboardPath, fullDashboardHTML, 'utf8');
console.log('📦 [1/2] Master dashboard.html compiled flawlessly.');

const serverPath = path.join(__dirname, 'server.js');
let serverContent = fs.readFileSync(serverPath, 'utf8');

const oldRouteMarker = "app.get('/app-apk', (req, res) => res.sendFile(path.join(__dirname, 'apk-simulator.html')));";
const completeInjectedRoutes = \`app.get('/app-apk', (req, res) => res.sendFile(path.join(__dirname, 'apk-simulator.html')));
...
if (serverContent.includes(oldRouteMarker) && !serverContent.includes('/statistics')) {
    serverContent = serverContent.replace(oldRouteMarker, completeInjectedRoutes);
    fs.writeFileSync(serverPath, serverContent, 'utf8');
    console.log('📡 [2/2] Server.js network endpoint routing sockets fully generated.');
}
